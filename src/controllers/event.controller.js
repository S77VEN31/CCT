...
export const getUserEventsAttendanceRequests = async (req, res) => {
    try {
        ...
        const events = await Event.find({ attendanceRequests: userId }).populate('owner');
        res.status(200).json(events);
    } catch (error) {
        const { code, name, message } = ErrorMessages.notEventsFound;
        res.status(code).json({ message, name });
...
export const addUserToEvent = async (req, res) => {
    try {
        ...
        // Send email to user
        await Promise.sendQRemail("https://picsum.photos/300/300", userEmail.email)
...
export const removeUserFromEvent = async (req, res) => {
    try {
        ...
        if (firstUser) {
            ...
            await Promise.sendQRemail("https://picsum.photos/300/300", userDocument.email)
        }
